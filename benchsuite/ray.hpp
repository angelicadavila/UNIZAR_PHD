...
// typedef enum { PLANE = 0, SPHERE = 1 } prim_type;
// enum class prim_type {
//                       PLANE = 0,
//                       SPHERE = 1
// };
...
// OpenCL requires 16-byte alignment (ie 4 floats) for its vector data types.
// Without
// the dummy_3 value, the Primitive struct will not copy from the host
// correctly.
// typedef struct
// Primitive
...
// Note - these enums were anonymous previously, and compiled fine under AMD's
// OpenCL drivers.
// Using Intels, the compiler throws an error with anonymous enums.
typedef enum raytype { ORIGIN = 0, REFLECTED = 1, REFRACTED = 2 } ray_type;

typedef enum primtype { PLANE = 0, SPHERE = 1 } prim_type;
...
int
raytrace(Ray* a_ray, Color* a_acc, float* a_dist, float4* point_intersect,
         int* result, local Primitive* primitives, int n_primitives)
{
  *a_dist = MAXFLOAT;
  int prim_index = -1;
  float prev_dist = MAXFLOAT;
  *result = MISS;
  // find nearest intersection
  for (int s = 0; s < n_primitives; s++) {
    if (intersect(&primitives[s], a_ray, a_dist)) {
      if (*a_dist < prev_dist) {
        prev_dist = *a_dist;
        *result = HIT;
        prim_index = s;
      ...
      if (primitives[l].is_light) {
        ...
        if (primitives[prim_index].m_diff > 0) {
          ...
          if (dot_prod > 0) {
            ...
            *a_acc +=
              diff * primitives[prim_index].m_color * primitives[l].m_color;
          }
        }
        ...
        if (primitives[prim_index].m_spec > 0) {
          ...
          if (dot_prod > 0) {
            float spec =
              native_powr(dot_prod, 20) * primitives[prim_index].m_spec * shade;
...
__kernel void
raytracer_kernel(__global Pixel* pixels, const int image_width,
                 const int image_height, const float camera_x,
                 const float camera_y, const float camera_z,
                 const float viewport_x, const float viewport_y,
                 __global float4* global_prims, const int n_primitives,
                 __local float4* local_prims)
{
  printf("raytracer %d", 0);
  // Copy primitives from global to local memory; GPU speedup was ~30% with
  // n_primitives 64
  event_t events[1];
  events[0] = async_work_group_copy(
    local_prims, global_prims,
    (size_t)((sizeof(Primitive) * n_primitives) / sizeof(float4)), 0);
  wait_group_events(1, events);

  __local Primitive* primitives = (__local Primitive*)local_prims;
  ...
  // Our viewport size can be different than the image size. This lets us
  // calculate
  // the stepping within the viewport relative to the stepping within the image.
  // IE with a viewport width of 6.0f and an image width of 800, each pixel is
  // 1/800 of 6.0f
  // or 0.0075f.
  ...
  // Initializes the ray queue. OpenCL has no support for recursion, so
  // recursive ray tracing calls
  // were replaced by a queue of rays that is processed in sequence. Since the
  // recursive calls were
  // additive, this works.
  ...
  // We use 3x supersampling to smooth out the edges in the image. This means
  // each pixel actually
  ...
  for (int tx = -1; tx < 2; tx++)
    for (int ty = -1; ty < 2; ty++) {
      ...
      float4 dir = NORMALIZE(
        (float4)(sx + dx * (tx / 2.0f), sy + dy * (ty / 2.0f), 0, 0) - camera);
      Ray r;
      r.origin = camera;
      r.direction = dir;
      r.weight = 1.0f;
      ...
      while (rays_in_queue > 0) {
        ...
        int prim_index = raytrace(&cur_ray, &ray_col, &dist, &point_intersect,
                                  &result, primitives, n_primitives);
        // reflected/refracted rays have different modifiers on the color of the
        // object
        switch (cur_ray.type) {
          case ORIGIN:
          ...
          case REFLECTED:
            acc += ray_col * cur_ray.weight *
                   primitives[cur_ray.origin_primitive].m_color *
        ...
        // JLB: added the condition (prim_index >=0)
        if ((cur_ray.depth < TRACEDEPTH) && (prim_index >= 0))
        // if (cur_ray.depth < TRACEDEPTH)
        {
          // reflection
          float refl = primitives[prim_index].m_refl;
          // JLB: comento esta parte pues no interesa ahora.
          //	if( primitives[prim_index].m_refl!=0)
          //	{
          //		n_r=-1000;
          //	}
          if (refl > 0.0f) {
          ...
          if (refr > 0.0f) {
            ...
            if (cosT2 > 0.0f) {
              ...
              new_ray.transparency =
                cur_ray.transparency *
                (exp(primitives[prim_index].m_color * 0.15f * (-dist)));
              PUSH_RAY(queue, new_ray, back_ray_ptr, rays_in_queue)
            }
          }
        }
  ...
  // Since we supersample 3x, we have to divide the total color by 9 to average
  // it.
  ...
  // pixels[y * image_width + x] = (Pixel)(n_r, n_r, n_r, n_r);
}
)";
  return move(kernel);
}
