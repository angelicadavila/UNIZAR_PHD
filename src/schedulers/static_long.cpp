...
StaticLongScheduler::StaticLongScheduler(WorkSplit wsplit)
  : m_wsplit(wsplit)
  , m_has_work(false)
  , m_sema_requests(1)
  , m_sema_callbacks(1)
...
StaticLongScheduler::~StaticLongScheduler()
{
...
void
StaticLongScheduler::endScheduler()
{
...
void
StaticLongScheduler::printStats()
{
  auto sum = 0;
...
void
StaticLongScheduler::notifyDevices()
{
...
void
StaticLongScheduler::saveDuration(ActionType action)
{
...
void
StaticLongScheduler::saveDurationOffset(ActionType action)
{
  lock_guard<mutex> lock(*m_mutex_duration);
  auto t2 = std::chrono::system_clock::now().time_since_epoch();
...
void
StaticLongScheduler::setWorkSize(size_t size)
{
  auto bound = 64;//128;
  size_t total = m_size;
...
void
StaticLongScheduler::setGWS(NDRange gws)
{
  m_gws = gws;
...
void
StaticLongScheduler::setLWS(size_t lws)
{
  m_lws = lws;
}

void
StaticLongScheduler::setWSBound(float ws_bound)
{
  m_ws_bound = ws_bound;
}

bool
StaticLongScheduler::hasWork()
{
  return m_size_rem_completed != 0;
}

void
StaticLongScheduler::waitCallbacks()
{
  m_sema_callbacks.wait(1);
}

void
StaticLongScheduler::waitRequests()
{
  m_sema_requests.wait(1);
}

void
StaticLongScheduler::notifyCallbacks()
{
  m_sema_callbacks.notify(1);
}

void
StaticLongScheduler::notifyRequests()
{
  m_sema_requests.notify(1);
}

void
StaticLongScheduler::setTotalSize(size_t size)
{
...
void
StaticLongScheduler::setRawProportions(const vector<float>& props)
{
...
tuple<size_t, size_t>
StaticLongScheduler::splitWork(size_t size, float prop, size_t bound)
{
...
void
StaticLongScheduler::setDevices(vector<Device*>&& devices)
{
 ...
 */
void
StaticLongScheduler::calcProportions()
{
  return; // NOTE(dyn)
 ...
 */
void
StaticLongScheduler::enq_work(Device* device)
{
  int id = device->getID();
  /*
...
void
StaticLongScheduler::preenq_work()
{
}

void
StaticLongScheduler::req_work(Device* device)
{
...
void
StaticLongScheduler::callback(int queue_index)
{
#if ATOMIC == 1
 ...
 */
int
StaticLongScheduler::getWorkIndex(Device* device)
{
  int id = device->getID();
  //lock_guard<mutex> guard(m_mutex_work);
...
Work
StaticLongScheduler::getWork(uint queue_index)
{
  //lock_guard<mutex> guard(m_mutex_work);
...
Device*
StaticLongScheduler::getNextRequest()
{
  Device* dev = nullptr;
#if ATOMIC == 1
