#define VECTOR 64 //number of random numbers per cycle
							    ...
							    __global float* restrict U_out, uint N,uint offset)
{
   ...
   for (ulong n=0; n<N/VECTOR+(MT_N/VECTOR+1); n++) {
      ...
      if(write_output){
         #pragma unroll VECTOR_DIV4
         for (int i=0; i<VECTOR_DIV4; i++) {
            U_out[i+next_chunk]=U[i];
            U_out[i+1*VECTOR_DIV4+next_chunk]=U[i+1*VECTOR_DIV4];
            U_out[i+2*VECTOR_DIV4+next_chunk]=U[i+2*VECTOR_DIV4];
            U_out[i+3*VECTOR_DIV4+next_chunk]=U[i+3*VECTOR_DIV4];
         }         
        next_chunk+=VECTOR; 
      }
    }
