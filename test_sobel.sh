name_file=test_watermark/datao21_only.txt
name_file2=test_watermark/datao21t_only.txt
...
name_file=test_sobel/model_dynamic.txt
  name_file2=test_sobel/modelt_dynamic.txt

for dev in 4 2 1 7 6 5 3
do
 #evaluating chunk size in performance of dynamic
   for chunk in {1..8000..80}
   #3907430 1953715 1302476 976857 781486 651238 55820434
   #312595200 3125952 625190 312595 156297 104198 
  # 4096 40960 204800 409600 4096000
 #  312595200 3125952 625190 312595 156297 104198
  ...
  echo $chunk_size
    echo $dev
    ./build/debug/clbalancer 1 $dev 5 0 1024 $chunk_size 0.52 21 &>>$name_file
    (tail -n2) < $name_file
    (tail -n2) < $name_file>>$name_file2
    echo '/build/debug/clbalancer 1 $dev 5 0 1024 $chunk_size 0.52 21' &>>$name_file
    sleep 5
    echo $sample
  done
  done
